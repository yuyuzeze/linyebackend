/*
    Azure SQL Database / SQL Server 向け初期スキーマ。
    対象 DB に対して実行する（SSMS、Azure Data Studio、sqlcmd）。

    スキーマ変更はバージョン付き DDL スクリプトで管理する（EF Migration は使用しない）。
    差分更新は database/scripts/ 配下に新しいスクリプトを追加する。
*/
